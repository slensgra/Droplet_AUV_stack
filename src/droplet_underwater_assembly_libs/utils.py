...
def get_roll_pitch_from_acceleration_vector(acceleration_vector):
    normed_vector = acceleration_vector / numpy.linalg.norm(acceleration_vector)

    accel_x, accel_y, accel_z = normed_vector

    pitch = math.atan(-accel_x / math.sqrt(pow(accel_y, 2) + pow(accel_z, 2)))
    roll = math.atan(accel_y / math.sqrt(pow(accel_x, 2) + pow(accel_z, 2)))
